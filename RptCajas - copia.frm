...
''''sqltext = "SELECT b.nombre,b.apellido,a.cajero, sum(a.precio) as Ventas " & _
    ...
    sqltext = sqltext & " COUNT(a.tipo_pago) as Z_COUNT FROM transac_pago as a "
    sqltext = sqltext & " WHERE a.cajero = " & rsCajeros!CAJERO
...
sqltext = sqltext & " COUNT(a.tipo_pago) as Z_COUNT "
sqltext = sqltext & " FROM transac_pago as a "
sqltext = sqltext & " WHERE a.tipo_pago <> 99 "
...
sqltext = "SELECT MID(a.TIPO,1,2) AS CORREC,DESCUENTO, "
sqltext = sqltext & " COUNT(a.lin) as Z_COUNT, SUM(a.precio) as valor "
sqltext = sqltext & " FROM TRANSAC as a WHERE MID(A.TIPO,1,1) <> ' ' "
...
cSQL = cSQL & rs00!VTA_TOT & "," & rs00!tot_hash & ")"
...
Do Until rsCajeros.EOF
    ...
    ' EN LOS AJUSTES
    sqltext = "SELECT a.cajero,a.tipo_pago,SUM(a.monto) AS Valor, "
    sqltext = sqltext & " COUNT(a.tipo_pago) as X_COUNT FROM transac_pago as a "
    sqltext = sqltext & " WHERE a.cajero = " & rsCajeros!CAJERO
    sqltext = sqltext & " AND a.tipo_pago <> 99 "
    sqltext = sqltext & " GROUP BY a.cajero,a.TIPO_PAGO"
...
'''''            "LUGAR DEL ERROR : " & Err.Source, vbCritical, "Número del Error: " & Err.Number
'''''    Resume Next
'''''End If
'''''End Sub
...
Else
    ShowMsg "IMPRESORA FISCAL NO ESTA HABILITADA", vbRed, vbBlue
End If
End Sub
...
If vResp = vbYes Then
    ...
    If Dir(cFactFile) = "" Then
        ...
        MsgBox "No hay archivo para imprimir", vbInformation, BoxTit
        Exit Sub
    Else
